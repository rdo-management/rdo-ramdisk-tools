...
def match(node, node_info):
    try:
        ...
        raise exc.LockFailedError(e.message, CONF.edeploy.lockname)
    ...
    try:
        ...
        raise exc.MatchFailedError(e.message, node.uuid)
    finally:
        sobj.save()
        sobj.unlock()
...
def main():
    ...
    try:
        ...
        err_msg = ('Failed to copy the state file: %s/state. '
                   'Error was: %s' % (CONF.edeploy.configdir, e.message))
    ...
    for node in nodes:
        ...
        except exc.LockFailedError as e:
        ...
        if node not in failed_nodes:
            try:
                ...
                err_msg = ('Failed to update node (%s). '
                           'Error was: %s' % (node.uuid, e.message))
                LOG.exception(err_msg)
