...
class TestMatch(TestBase):
    ...
    def test_lock_failed(self, state_mock, mock_facts):
        self.assertRaises(exc.LockFailedError, match.match,
...
class TestMain(TestBase):
    ...
    def test_copy_failed(self, mock_copy, mock_ic, mock_log):
        ...
        self.assertRaisesRegexp(SystemExit, '^Failed to copy.*boom$',
                                match.main)
        self.assertFalse(mock_log.error.called)
        self.assertTrue(1, mock_log.exception.call_count)

    @mock.patch.object(match, 'match', autospec=True,
                       side_effect=exc.LockFailedError('boom', 'boom.lock'))
    @mock.patch.object(match, '_copy_state', lambda: None)
    def test_lock_failed(self, mock_match, mock_ic, mock_log):
        mock_ic.return_value = self.mock_client
        self.assertRaisesRegexp(exc.LockFailedError, '.*process.*lock.*boom$',
                                match.main)
        self.assertFalse(mock_log.error.called)
        self.assertTrue(1, mock_log.exception.call_count)
    ...
    @mock.patch.object(match, '_copy_state', lambda: None)
    def test_match_failed(self, mock_match, mock_ic, mock_log):
        mock_ic.return_value = self.mock_client
        match.main()
